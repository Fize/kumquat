...
func (c *UserController) List() (gin.HandlerFunc, error) {
	return func(ctx *gin.Context) {
		...
		if principal.Admin {
			...
			users, total, err = c.svc.ListScoped(ctx.Request.Context(), principal.ModulePublicID, page, size)
		}
		if err != nil {
...
func (c *UserController) Get() (gin.HandlerFunc, error) {
	return func(ctx *gin.Context) {
		...
		principal := businessPrincipal(ctx)
		if !principal.Admin {
			allowed, scopeErr := c.svc.CanAccess(ctx.Request.Context(), principal.ModulePublicID, uint(id))
