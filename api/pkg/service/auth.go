...
// InitAdminUser creates or upgrades the explicitly configured bootstrap user.
// The API never supplies a default password.
func (s *AuthService) InitAdminUser(ctx context.Context, username, email, password string) error {
	if username == "" || password == "" {
		return errors.New("bootstrap admin username and password are required")
	}
	// Find the admin role
	var adminRole model.Role
	if err := s.db.WithContext(ctx).Where("name = ?", model.RoleAdmin).First(&adminRole).Error; err != nil {
		return fmt.Errorf("admin role not found: %w", err)
	}

	// Check if admin user already exists
	adminUser, err := s.repo.GetByUsername(ctx, username)
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return fmt.Errorf("failed to check admin user: %w", err)
	}

	if adminUser != nil {
		return errors.New("bootstrap admin username already exists; refusing to promote a preclaimed account")
	}
	...
	log.InfoContext(ctx, "admin user created", "user_id", user.ID, "username", username)
	return nil
}
