...
func TestModuleService_Create_ParentAtMaxLevel(t *testing.T) {
	...
	level5 := &model.Module{PublicID: model.NewPublicID("module"), Name: "level5", Level: model.MaxModuleLevel, Sort: 0, Path: "/a/b/c/d/level5"}
...
func TestModuleService_RenameUpdatesDescendantPaths(t *testing.T) {
	db := setupTestDB(t)
	svc := newModuleServiceFromDB(db)
	root := createTestModule(t, db, "old", nil, 0)
	child := createTestModule(t, db, "child", &root.ID, 0)
	_, err := svc.Update(context.Background(), root.ID, "new", 0)
	require.NoError(t, err)
	var got model.Module
	require.NoError(t, db.First(&got, child.ID).Error)
	assert.Equal(t, "/new/child", got.Path)
}

func TestModuleService_DeleteRejectsReferencedDescendant(t *testing.T) {
	db := setupTestDB(t)
	svc := newModuleServiceFromDB(db)
	root := createTestModule(t, db, "root", nil, 0)
	child := createTestModule(t, db, "child", &root.ID, 0)
	project := model.Project{Name: "p", ModuleID: child.ID}
...
func TestModuleServiceAdversarialNamesDoNotExpandAncestry(t *testing.T) {
	db := setupTestDB(t)
	svc := newModuleServiceFromDB(db)
	root := createTestModule(t, db, "%_\\", nil, 0)
	child := createTestModule(t, db, "child", &root.ID, 0)
	other := createTestModule(t, db, "other", nil, 0)
	otherChild := createTestModule(t, db, "unrelated", &other.ID, 0)
	_, err := svc.Update(context.Background(), root.ID, "renamed", 0)
	require.NoError(t, err)
	var gotChild, gotOther model.Module
	require.NoError(t, db.First(&gotChild, child.ID).Error)
	require.NoError(t, db.First(&gotOther, otherChild.ID).Error)
	assert.Equal(t, "/renamed/child", gotChild.Path)
	assert.Equal(t, "/other/unrelated", gotOther.Path)
	project := model.Project{Name: "p", ModuleID: otherChild.ID}
	require.NoError(t, db.Omit("config").Create(&project).Error)
