...
- **Intelligent Scheduling**: Select target clusters with Spread, BinPacking, and Affinity policies
- **Dual Connection Mode**: Support both direct Hub access and reverse Edge connectivity
...
Users submit applications, clusters, and workspace configuration through the Kumquat API
or `kumctl`. Engine runs in the Hub cluster, schedules and distributes workloads, and
manages member clusters through either direct Hub access or Edge tunnels.
...
| **Scheduler** | Engine | Selects target clusters based on policy and cluster state |
...
| **API** | API | User entry point for auth, projects/modules, and resource operations |
...
| [apiserver/](apiserver/) | API | User-facing API gateway for identity, projects/modules, and resource operations |
| [kumctl/](kumctl/) | Kumctl | API-only command-line entry point for users and agents |

## Quick Start

The local Kind demo starts the Hub cluster, Engine, API, and sample workloads so you can
...
| Addon | Purpose |
|-------|---------|
| **mcs-lighthouse** | Submariner-based cross-cluster service discovery and networking |
| **kruise-rollout** | Cross-cluster progressive delivery coordination |
| **victoriametrics** | Multi-cluster monitoring collection and aggregation |
...
|----------|-------------|
| [Documentation Home](docs/en/index.md) | English technical documentation entry |
