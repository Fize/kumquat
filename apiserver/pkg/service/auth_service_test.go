...
func TestInitAdminUserRefusesPreclaimedUsername(t *testing.T) {
	db := setupTestDBWithRoles(t)
	createTestUser(t, db, "bootstrap", "attacker@example.test", "attacker-password", 3)
	err := newAuthServiceFromDB(db).InitAdminUser(context.Background(), "bootstrap", "admin@example.test", "trusted-password")
	require.ErrorContains(t, err, "refusing to promote")
	var user model.User
	require.NoError(t, db.Where("username = ?", "bootstrap").First(&user).Error)
	assert.Equal(t, uint(3), user.RoleID)
	assert.True(t, user.CheckPassword("attacker-password"))
}
