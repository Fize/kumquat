...
// setupTestDB creates an isolated real-MySQL database with auto-migration.
func setupTestDB(t *testing.T) *gorm.DB {
	db := testdb.Open(t)
	err := db.AutoMigrate(&model.User{}, &model.Role{}, &model.Permission{}, &model.Module{}, &model.Project{}, &model.ResourceRecord{}, &model.Operation{}, &model.OutboxEvent{})
...
func createTestModule(t *testing.T, db *gorm.DB, name string, parentID *uint, sort int) *model.Module {
	m := &model.Module{PublicID: model.NewPublicID("module"), Name: name, ParentID: parentID, Sort: sort}
