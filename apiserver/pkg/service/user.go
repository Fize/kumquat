...
func (s *UserService) ListScoped(ctx context.Context, modulePublicID string, page, size int) ([]model.User, int64, error) {
	var root model.Module
	if err := s.db.WithContext(ctx).Where("public_id = ?", modulePublicID).First(&root).Error; err != nil {
	...
	query := s.db.WithContext(ctx).Model(&model.User{}).Where("module_id IN ?", ids)
	var total int64
	if err := query.Count(&total).Error; err != nil {
		return nil, 0, apperr.WrapCode(apperr.CodeInternal, err)
	}
	var users []model.User
	if err := query.Preload("Role").Preload("Module").Offset((page - 1) * size).Limit(size).Find(&users).Error; err != nil {
		return nil, 0, apperr.WrapCode(apperr.CodeInternal, err)
	}
	return users, total, nil
}

func (s *UserService) CanAccess(ctx context.Context, modulePublicID string, userID uint) (bool, error) {
	var root model.Module
	if err := s.db.WithContext(ctx).Where("public_id = ?", modulePublicID).First(&root).Error; err != nil {
		return false, err
	}
	...
	var count int64
	if err := s.db.WithContext(ctx).Model(&model.User{}).Where("id = ? AND module_id IN ?", userID, ids).Count(&count).Error; err != nil {
		return false, err
	}
	return count == 1, nil
}
...
func (s *UserService) Create(ctx context.Context, username, email, password, nickname string, roleID uint, moduleID *uint) (*model.User, error) {
	...
	if moduleID != nil {
		var count int64
		if err := s.db.WithContext(ctx).Model(&model.Module{}).Where("id = ?", *moduleID).Count(&count).Error; err != nil {
			return nil, apperr.WrapCode(apperr.CodeInternal, err)
		}
		if count == 0 {
